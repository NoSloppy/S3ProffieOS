...
/*
 Sabertrio Proffie Preconfiguration Version: 2.0
 ----------------------------------------------------
 This file is used to compile and upload the 
 Sabertrio Proffie Preconfiguration firmware into 
 your Proffie saber through Arduino. By default, it
 is referencing to "config/s3config.h" which is the
 firmware for a standard single bladed Neo Proffie 
 Saber without an OLED screen. 

 If your saber has an OLED screen or is a Krosgaard,
 comment OUT #define CONFIG_FILE "config/s3config.h"
 by adding "//" before it.

 Then, comment IN the relevent reference file by
 removing the "//". Ensure you do not have multiple
 reference files active. 
